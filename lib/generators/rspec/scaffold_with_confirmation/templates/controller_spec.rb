...
<% end -%>
  ...
  describe "POST preview_creation" do
    describe "with valid params" do
      it "assigns a <%= file_name %> as @<%= file_name %>" do
        <%= stub orm_class.build(class_name, params) %> { <%= mock_file_name %> }
        <%= mock_file_name %>.should_receive(:valid?).and_return(true)
        post :preview_creation, :<%= file_name %> => <%= params %>
    ...
    describe "with invalid params" do
      it "assigns a <%= file_name %> as @<%= file_name %>" do
        <%= stub orm_class.build(class_name, params) %> { <%= mock_file_name %> }
        <%= mock_file_name %>.should_receive(:valid?).and_return(false)
        post :preview_creation, :<%= file_name %> => <%= params %>
    ...
    describe "resetting" do
      it "redirect" do
        post :preview_creation, :reset => true, :<%= file_name %> => <%= params %>
        response.should redirect_to(new_<%= table_name.singularize %>_url)
      end
    end
  end
  ...
  describe "POST preview_update" do
    describe "with valid params" do
      it "assigns a <%= file_name %> as @<%= file_name %>" do
        <%= stub orm_class.find(class_name, "37".inspect) %> { <%= mock_file_name %> }
        <%= mock_file_name %>.should_receive(:attributes=).with(<%= params %>)
        <%= mock_file_name %>.should_receive(:valid?).and_return(true)
        post :preview_update, :id => "37", :<%= file_name %> => <%= params %>
        assigns[:<%= file_name %>].should equal(mock_<%= file_name %>)
      end
    end
    ...
    describe "resetting" do
      it "redirect" do
        <%= stub orm_class.find(class_name, "37".inspect) %> { <%= mock_file_name %> }
        post :preview_update, :reset => true, :id => "37", :<%= file_name %> => <%= params %>
        response.should redirect_to(edit_<%= table_name.singularize %>_url(mock_<%= file_name %>))
      end
    end
  end
